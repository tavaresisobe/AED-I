...
{ // se a fila estiver vazia insere o primeiro elemento
    ...
    { filaAux->primeiro = 0 ;
      ...
      filaAux->tamanhofila ++ ;
        ...
        filaAux->ultimo = posicao;
        filaAux->tamanhofila ++ ;
    }else printf("overflow: fila cheia");
}

void removerElemento (tipoFC *filaAux)
{
    if (filaVazia(filaAux) == 0)
    {
        filaAux->elementos[filaAux->primeiro] = 0 ;
...
void listarFila2(tipoFC *filaAux)
{
    int i ;
...
int main ()
{
    ...
    if (x > 0 && x <= 100)
    {
        ...
        for (i = 0 ; i < x ; i++)
        {
            ...
            {
                scanf ("%d", &valor) ;
                insereElemento(&filaEsC, valor) ;
                listarFila2(&filaEsC) ;
                listarFila(&filaEsC) ;
            }
            if (op == 2)
            {
                removerElemento(&filaEsC) ;
